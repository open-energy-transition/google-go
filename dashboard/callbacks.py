...
def register_callbacks(app, data_loader):
    ...
    def update_ci25_summary(year, scenario, metric):
    ...
    def update_ci50_summary(year, scenario, metric):
    ...
    def update_cinoadd_summary(year, scenario, metric):
    ...
    @app.callback(
        ...
        [Input('comp-year-selector', 'value'),
         ...
         Input('comp-plot-type-selector', 'value')]
    )
    def update_comparison_plot(year, metric, scenarios, plot_type):
        """Update main comparison plot"""
        return create_comparison_plot(year, metric, scenarios, plot_type,
    ...
    def update_comparison_details(year, metric):
        """Update individual scenario detail plots"""
        fig1 = create_scenario_detail_plot('CI_25', year, metric, data_loader, color_mapper)
        fig2 = create_scenario_detail_plot('CI_50', year, metric, data_loader, color_mapper)
        fig3 = create_scenario_detail_plot('CI_noadd', year, metric, data_loader, color_mapper)
    ...
    def update_difference_plot(year, metric, scenarios):
        """Update difference analysis plot"""
        return create_difference_plot(year, metric, scenarios, data_loader, color_mapper)
    ...
    def update_comparison_table(year, metric, scenarios):
        """Update comparison summary table"""
        return create_comparison_table(year, metric, scenarios, data_loader)
...
def create_summary_stats(scenario, year, scenario_name, metric, data_loader):
    ...
    try:
        ...
        return html.Div([
            html.P(f"Total: {total:.2f}"),
            html.P(f"Mean: {mean:.2f}"),
            html.P(f"Std Dev: {std:.2f}"),
            html.P(f"Min: {min_val:.2f}"),
            html.P(f"Max: {max_val:.2f}"),
            html.P(f"Number of carriers: {len(data_series)}")
        ])
...
def create_comparison_plot(year, metric, scenarios, plot_type, data_loader, color_mapper):
    """Create comparison plot across scenarios"""
    try:
        if not scenarios or not year or not metric:
            return create_empty_figure("Please select year, metric, and at least one scenario")

        fig = go.Figure()

        for scenario in scenarios:
            # Get first sub-scenario for each main scenario
            stats = data_loader.get_summary_stats(scenario)
            sub_scenarios = stats.get('scenarios', [])

            if not sub_scenarios:
                continue

            # Use first sub-scenario as representative
            sub_scenario = sub_scenarios[0]

            df = data_loader.get_data(scenario, year=year, scenario_name=sub_scenario, metric=metric)
        ...
        fig.update_layout(
            title=f"Comparison: {metric} ({year})",
            xaxis_title="Carrier",
            yaxis_title="Value",
            template="plotly_white",
            barmode='group',
...
def create_scenario_detail_plot(scenario, year, metric, data_loader, color_mapper):
    """Create detail plot for a single scenario"""
    try:
        stats = data_loader.get_summary_stats(scenario)
        sub_scenarios = stats.get('scenarios', [])

        if not sub_scenarios or not year or not metric:
            return create_empty_figure("No data available")

        # Use first sub-scenario
        sub_scenario = sub_scenarios[0]
...
def create_difference_plot(year, metric, scenarios, data_loader, color_mapper):
    ...
    try:
        ...
        for scenario in scenarios:
            ...
            df = data_loader.get_data(scenario, year=year, scenario_name=sub_scenarios[0], metric=metric)
        ...
        fig.update_layout(
            title=f"Difference Analysis: {metric} ({year})",
            xaxis_title="Carrier",
...
def create_comparison_table(year, metric, scenarios, data_loader):
    """Create comparison table"""
    try:
        if not scenarios or not year or not metric:
            return html.P("Please select year, metric, and scenarios")
