The following features are implemented through specialized Python scripts (`strip_network.py`, `add_certificate.py`, and parts of `solve_network.py`) developed specifically for the Google-GO project within the PyPSA-Eur workflow. These scripts extend the standard PyPSA functionalities to address the unique modeling requirements of Guarantee of Origin (GoO) certificates and simplified network analysis.
...
The `add_certificate.py` script integrates the Guarantee of Origin (GoO) certificate system into the energy system models. It achieves this by creating "virtual" representations of power plants and storage units, and establishing a market for these certificates within the PyPSA network.
